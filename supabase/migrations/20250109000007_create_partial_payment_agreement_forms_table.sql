-- Create partial_payment_agreement_forms table
...
-- Create RLS policies
CREATE POLICY "Users can view partial payment agreement forms for patients they have access to" ON partial_payment_agreement_forms
    FOR SELECT USING (auth.uid() IS NOT NULL);

CREATE POLICY "Users can insert partial payment agreement forms" ON partial_payment_agreement_forms
    FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Users can update partial payment agreement forms" ON partial_payment_agreement_forms
    FOR UPDATE USING (auth.uid() IS NOT NULL);

CREATE POLICY "Users can delete partial payment agreement forms" ON partial_payment_agreement_forms
...
CREATE TRIGGER update_partial_payment_agreement_forms_updated_at
